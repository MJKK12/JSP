...
$(document).ready(function() {
   ...
   IMP.init('imp42034783');  // 중요!!  아임포트에 가입시 부여받은 "가맹점 식별코드". 
	   ...
	   */
		...
		*/
		//	opener.location.href = "javascript:goCoinUpdate('${requestScope.userid}','${requestScope.coinmoney}');";
			window.opener.goCoinUpdate('${requestScope.userid}','${requestScope.coinmoney}');	// login.jsp 에 있는 함수
			// DB 를 바꿔야 하므로 누구인지 (userid) 알아와야 한다.
		//  $(opener.location).attr("href", "javascript:goCoinUpdate('${requestScope.userid}','${requestScope.coinmoney}');");
			
		    self.close();	// 성공하면 부모창에 있는 함수를 호출하고, 팝업창을 닫는다.
