...
<c:set var="name1" value="쌍용강북" scope="page"></c:set>
<c:set var="name2" value="교육센터" /> <%-- 생략하면 scope="page" 이다. --%>

<c:set var="name_scope_page" 	value="${name1}&nbsp;${name2}" scope="page"></c:set>
<c:set var="name_scope_request" value="${name2}" 			   scope="request"></c:set>
<c:set var="name_scope_session" value="${name1}&nbsp;${name2}" scope="session"></c:set>
...
<%--
	scope="page" 로 선언된 변수의 사용범위는 선언되어진 jsp 파일 및 선언되어진 파일 속에
	<%@ include file="ㅇㅇㅇ.jsp" %> 되어진 ㅇㅇㅇ.jsp 파일내에서도 사용되어진다.
...
<%-- === <c:set var="변수명" value="${값}" scope="" /> 를 테스트 하기 위해서 사용하는 것 끝. === --%>
		...
		<div class="collapse navbar-collapse" id="collapsibleNavbar">
		  <ul class="navbar-nav" style="font-size: 16pt;">
			...
			<li class="nav-item active">          
 			...
 			<%-- 관리자(admin) 일 때만 보여주겠다. --%>
          <c:if test="${not empty sessionScope.loginuser && sessionScope.loginuser.userid eq 'admin' }">	<%-- 로그아웃이 되면 보여지면 안된다. == session 이 empty 면 안된다.(not empty 여야 한다.) --%>
 				<li class="nav-item dropdown">	<%-- 로그인이 되어있어야 하고 && userid 가 admin 이어야 한다. (eq 또는 == 사용 가능. ↔ ne 또는 != 사용 가능.) --%>
                 ...
                 <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                    ...
                    <a class="dropdown-item text-info" href="<%= ctxPath %>/shop/orderList.up">전체주문내역</a> <%-- 관리자는 모든 회원들의 주문내역을 봐야하기 때문에 where 절만 없을 뿐이다. --%>
                 	</div>
            	</li> 
           </c:if>	 

 		  <%-- 관리자가 아닌 일반 사용자로 로그인 했을 때 (관리자는 장바구니가 없음) --%>
          <c:if test="${not empty sessionScope.loginuser and sessionScope.loginuser.userid ne 'admin' }">	<%-- 로그아웃이 되면 보여지면 안된다. == session 이 empty 면 안된다.(not empty 여야 한다.) --%>
 				<li class="nav-item dropdown">	<%-- 로그인이 되어있어야 하고 && userid 가 admin 이어야 한다. (eq 또는 == 사용 가능. ↔ ne 또는 != 사용 가능.) --%>
                 <a class="nav-link dropdown-toggle menufont_size text-info" href="#" id="navbarDropdown" data-toggle="dropdown"> 
                 ...
                 <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                    ...
                    <a class="dropdown-item text-info" href="<%= ctxPath %>/shop/orderList.up">나의주문내역</a> <%-- 사용자는 내 주문내역을 봐야하기 때문에 where 절 == 내 id 이다. --%>
                 </div>
            	</li> 
           </c:if>
    ...
    <div class="container-fluid" style="position: relative; top:85px; padding: 0.1% 2.5%;">
      
	  <div class="row">
		 <div class="col-md-3" id="sideinfo">
		 
			<%-- 유튜브 넣기 header.jsp 에만 있음 --%>
	        <div class="row">
			...
			<div style="height: 200px; text-align: left; padding: 20px;">
				...
				<%@ include file="/WEB-INF/login/login.jsp" %>	<%-- 로그인 폴더를 넣는다. --%>
			</div>
