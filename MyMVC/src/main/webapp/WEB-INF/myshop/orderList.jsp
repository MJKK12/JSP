...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#btnDeliverStart").click(function(){
			...
			// 배송하기 및 배송완료 관련 모든 input 태그 비활성화 시키기(비활성화 된 checkbox 나 hidden 모두 form 태그로 보낼 수 없다.)
			...
			// <input type="checkbox" ~~~> 의 다음 다음 <> 태그를 살린다(disabled 를 false로)
			
			// DB에 상태값 insert 하고 문자로 배송관련내용 발송 (DB에서 배송상태를 1-->2로 바꾼다.)
	...
	function openMember(odrcode){
		const url = "<%= ctxPath%>/shop/odrcodeOwnerMemberInfo.up?odrcode="+odrcode;
...
<div class="container-fluid" style="border: solid 0px red">
	<div class="my-3">
		<c:set var="userid" value="${(sessionScope.loginuser).userid}" />	<%-- 변수 var 로 설정한 후 아래에서 활용한다. --%>
		
		<c:if test='${userid eq "admin"}'>	<%-- 관리자 일 때 --%>
			<p class="h4 text-center">&raquo;&nbsp;&nbsp;주문내역 전체목록&nbsp;&nbsp;&laquo;</p>
		</c:if>
		
		<c:if test='${userid ne "admin"}'>	<%-- 관리자가 아닐때 --%>
					...
					<c:forEach var="odrmap" items="${requestScope.orderList}" varStatus="status">
						...
						<tr>
							<td align="center"> <%-- 주문코드(전표) 출력하기. 
								...
								<c:if test='${userid eq "admin"}'> <%-- 관리자로 로그인 했을 때 전표번호 클릭시 회원상세정보 조회 --%>
							...
							<td style="cursor:pointer;" onclick="javascript:location.href='/MyMVC/shop/prodView.up?pnum=${odrmap.FK_PNUM}'">  <%-- === 제품정보 넣기 === --%>
								...
								<div style="display: flex; padding-top: 10px; justify-content: space-between;">
								    ...
								    --%>
									<div style="width: 44%;">
									    <img src="/MyMVC/images/${odrmap.PIMAGE1}" width="100%" />  <%-- 제품이미지1 --%>
									</div>
									<div style="width: 54%;">
