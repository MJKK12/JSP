...
<script type="text/javascript">
   ...
   $(document).ready(function() {
     ...
     goLikeDislikeCount(); // 좋아요, 싫어요 갯수를 보여주도록 하는 것이다. (DB 에서 select 로 얻어온다.)
     ...
     $.ajax({
           ...
           type:"GET",
           data:{"fk_pnum":"${requestScope.pvo.pnum}"	// 제품 번호
                ,"fk_userid":"${sessionScope.loginuser.userid}"},	// 너 누구니? 사용자를 알아와야 한다.
           dataType:"JSON",	// 결과물을 JSON 으로 받는다.
         
           async:false,   // 동기처리(결과물을 가지고 올 때 까지 그 다음으로 넘어가지 않음)
  ...
  // alert("확인용 동기처리와 비동기 처리 차이점 확인용 : " + isOrderOK);
        ...
        --%>
            ...
            $.ajax({
               ...
               success:function(json){ // {"n":1} 또는 {"n":-1} 또는  {"n":0}
                  ...
                  // 동일한 제품에 대하여 동일한 회원이 제품후기를 2번 쓰려고 한 경우 unique 제약에 위배됨 
      ...
      if(isOrderOK == false) {  // 해당 제품을 구매하지 않은 경우 이라면 
    	...
    	//  alert("제품을 구매한 적이 있습니다.");
    	  ...
    	  $.ajax({
            ...
            success:function(json) {
            	// alert(json.msg);
                   swal(json.msg);	// 결과물(json) 을 msg로 달라는 것. (sweet alert)
                   goLikeDislikeCount();	// 좋아요 변경된 갯수를 다시 읽어오자.
            },
            error: function(request, status, error){
               alert("code: "+request.status+"\n"+"message: "+request.responseText+"\n"+"error: "+error);
            }
      ...
      if(isOrderOK == false) {  // 해당 제품을 구매하지 않은 경우 이라면 
         ...
         $.ajax({
            ...
            success:function(json) {
               // alert(json.msg);
                  swal(json.msg);
                  goLikeDislikeCount();	// 싫어요 변경된 갯수를 다시 읽어오자.
            },
            error: function(request, status, error){
               alert("code: "+request.status+"\n"+"message: "+request.responseText+"\n"+"error: "+error);
            }
   ...
   function goLikeDislikeCount() {
      ...
      $.ajax({
         url:"<%= request.getContextPath()%>/shop/likeDislikeCount.up",	<%-- 문서 로딩되자마자 url 가서 보여쥬자. --%>
      // type:"GET",
         data:{"pnum":"${requestScope.pvo.pnum}"},	// 어떤 제품에 대한 좋아요,싫어요 인지 알아야 하므로 pnum 을 보낸다.
         dataType:"JSON", 
         success:function(json) {
            $("span#likeCnt").html(json.likecnt);		// 좋아요에 투표된 갯수를 표시해준다
            $("span#dislikeCnt").html(json.dislikecnt);	// 싫어요에 투표된 갯수를 표시해준다
         },
         error: function(request, status, error){
            alert("code: "+request.status+"\n"+"message: "+request.responseText+"\n"+"error: "+error);
         }
   ...
   function goCommentListView() {
      ...
      $.ajax({
         ...
         success:function(json) {
            ...
            if (json.length > 0) {    // 후기가 있다면 보여주자.
               $.each(json, function(index, item){ // item 이 json 객체 하나하나를 말하고 있는것 (댓글 한 행..)
                  ...
                  var loginuserid = "${sessionScope.loginuser.userid}";
                   ...
                   if( loginuserid == "") {		// 로그인을 안한 상태
                      html += "<div class='customDisplay spacediv'>&nbsp;</div>";
                   }      
                   else if( loginuserid != "" && writeuserid != loginuserid ) {	// 로그인은 했지만 쓴사람!=작성자id
                      html += "<div class='customDisplay spacediv'>&nbsp;</div>";
                   }    
                   else if( loginuserid != "" && writeuserid == loginuserid ) {	// 로그인도 하고 쓴사람==작성자id
   ...
   function updateMyReview(index,review_seq) {
	...
	//  alert(review_contents);
      ...
      // alert($("textarea#edit_textarea").val()); // 수정할 제품후기 내용
         ...
         $.ajax({
            url:"<%= request.getContextPath()%>/shop/commentUpdate.up",
            type:"POST",
            data:{"review_seq":review_seq
   ...
   function goCart() {
      ...
      const regExp = /^[0-9]+$/;  // 숫자만 체크하는 정규표현식	(주문개수 란에 숫자외에 글자를 넣지 못하도록 함.)
      let oqty = frm.oqty.value;	// form 태그에서 value 를 읽어온다.
      const bool = regExp.test(oqty);
      
      if(!bool) {	// false 일 때 alert 를 띄운다.
         // 숫자 이외의 값이 들어온 경우 
...
<div style="width: 95%;">
   ...
   <div class="row my-3">
      ...
      <div class="col-md-6 pl-5">
          <ul class="list-unstyled">
            ...
            <li>제품설명서: 
              <c:if test="${requestScope.pvo.prdmanual_orginFileName ne '없음'}">	<%-- 넘어온 prdmanual_orginFileName 값이 '없음' (DAO 에서 nvl(prdmanual_orginFileName,'없음')으로 해둠) --%>
                 <a href="<%= ctxPath%>/shop/fileDownload.up?pnum=${pvo.pnum}">${pvo.prdmanual_orginFileName}</a>	<%-- 첨부된 파일 다운받기 --%>
              </c:if>
   ...
   <c:if test="${not empty requestScope.imgList}">	<%-- null 또는 size 가 0 이다. (not empty) --%>
      <div class="row">
    ...
    <div class="row">
        <div class="col-lg-10">
          <form name="commentFrm">
              ...
              <input type="hidden" name="fk_userid" value="${sessionScope.loginuser.userid}" />		<%-- 로그인된 아이디 --%>
              <input type="hidden" name="fk_pnum" value="${requestScope.pvo.pnum}" />				<%-- 보고있는 제품 번호 --%>
          </form>
       </div>
