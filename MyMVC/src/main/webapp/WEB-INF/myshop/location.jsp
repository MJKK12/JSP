...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=001439ebb0ff67b6ace22acc241bbbeb"></script> 
 ...
 <script type="text/javascript">
  $(document).ready(function(){
   ...
   mapobj.addControl(mapTypeControl, kakao.maps.ControlPosition.TOPRIGHT);	// TOPLEFT 일 시 왼쪽 위
   ...
   if (navigator.geolocation) {
      ...
      //   console.log("현위치의 위도: "+latitude+", 현위치의 경도: "+longitude);
         ...
         mapobj.setCenter(locPosition);	// 이 라인을 생략하게 되면 기본값 위도,경도에 설정된다.
      });
   }   
   else {
   ...
   const positionArr = [];	// 마커가 복수개 이므로 배열을 사용한다. (DB 에서 불러온 것들을 배열속에 넣어주었다.)
   
   $.ajax({
      url:"/MyMVC/shop/locationJSON.up",
      async:false, // !!!!! 지도는 비동기 통신이 아닌 동기 통신으로 해야 한다.!!!!!! 위경도를 알아와야 하고, success 이후에 진행되어야 하므로 동기통신
      dataType:"json",
   ...
   for(let i=0; i<positionArr.length; i++) {
      ...
      // == 매장마커 생성하기 == // positionArr 에 위,경도 존재
