	 ...
	 // Connection conn 은 오라클 데이터베이스 서버와 연결을 맺어주는 객체
		...
		try {
		  ...
		  */ 
			...
			cstmt = conn.prepareCall("{call pcd_student_select_one(?,?,?,?,?,?,?)}");	// 프로시져를 호출해주는 우편배달부이다.(오라클로부터) // 파라미터가 String 타입이므로 "" 안에 기입.
         ...
         */
			...
			cstmt.setString(1, stno);		// 숫자 1은 프로시저 파라미터중 첫번째 파라미터인 IN 모드의 ?를 말한다. ==> IN 모드인 첫번째에 학번을 넣어주겠다.
			...
			if( cstmt.getString(2) == null ) {			// OUT 모드의 2번째 결과물을 끄집어오겠다. (get 으로)
