    ...
    -- Table JDBC_TBL_CLASS이(가) 생성되었습니다.
    ...
    nocycle
    nocache;
    -- Sequence JDBC_SEQ_CLASSNO이(가) 생성되었습니다.

    insert into jdbc_tbl_class(classno, classname, teachername) 
    values(jdbc_seq_classno.nextval, '자바웹프로그래밍A', '김샘');    -- 학급번호(classno)는 sequence 로 준다.
    ...
    -- Table JDBC_TBL_STUDENT이(가) 생성되었습니다.
    ...
    nocycle
    nocache;
    -- Sequence JDBC_SEQ_STNO이(가) 생성되었습니다.
    ...
    */
    ...
    where S.stno = 9001;         -- procedure 로 만들어서 S.stno 만 바뀌게끔 하자.
    ...
    ,o_name             OUT jdbc_tbl_student.name%type              -- o 는 OUT 모드(출력모드)
    ,o_tel              OUT jdbc_tbl_student.tel%type
    ,o_addr             OUT jdbc_tbl_student.addr%type
    ,o_registerdate     OUT varchar2
    ,o_classname        OUT jdbc_tbl_class.classname%type           -- classname 컬럼의 타입을 쓰겠다
    ...
    ----------------------------------------------------------------------------------------
/*
    -- *** jdbc_tbl_student 테이블에 insert 할 수 있는 요일명 과 시간을 제한하겠습니다. *** --
    
    월,화,수,목,금 만 가능하며, 또한 월,화,수,목,금 이더라도 
    시간이 오전 9시 부터 오후 1시 59분 59초 까지만 가능하도록 한다.
    insert 할 수 없는 요일명 이거나 시간대 라면 "영업마감 이므로 insert 를 할 수 없습니다." 라는 오류메시지를 띄우도록 한다.
*/
    ...
    begin
        ...
        exception
            when error_dayTime then
                 raise_application_error(-20005,'>> 영업시간(월~금 09시~17시이전)마감 이므로 insert 를 할 수 없습니다. <<');       -- 사용자가 정의하는 error 범위 (-20001~20999)
